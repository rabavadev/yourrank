...
- The bot dashboard's inline `<script>` tag generated by `dashboard-views/client-script.ts` may not receive a `nonce` attribute, causing the browser's CSP to block it. The symptom: no `load()` function is defined, all page panels remain visible, and data stays "Loading…".

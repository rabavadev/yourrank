...
export function broadcastsPanel(): string {
  ...
  <div class="lb-bento" data-page="broadcasts">
    <div class="lb-widget lb-widget--full">
      <div class="mb-md"><h2>Broadcast to subscribers</h2></div>
      ...
      <div id="bcComposer" class="d-flex flex-col gap-24 mt-md" hidden>
        <div class="bc-step" data-step="1">
          <div class="mb-sm"><h3>1. Composer</h3></div>
          <div class="d-flex flex-col gap-8 broadcast-message-fields">
            <label class="sr-only" for="bcBody">Message</label>
            <textarea class="v3-input" id="bcBody" rows="3" aria-errormessage="bcBody-error" placeholder="Message to all your bot's subscribers — use {name} to include the subscriber's first name"></textarea>
            <span id="bcBody-error" class="field-err" role="alert"></span>
            <label class="sr-only" for="bcImage">Image URL</label>
            <input class="v3-input" id="bcImage" type="url" placeholder="Image URL (optional) — shown above the message" />
          </div>
        </div>

        <div class="bc-step" data-step="2">
          <div class="mb-sm"><h3>2. Subscribers</h3></div>
          <div class="d-flex flex-col gap-8 mb-md broadcast-bot-field">
            <label for="bcBotSelect" class="muted text-xs">From bot</label>
          ...
          <div class="d-flex flex-wrap gap-16 broadcast-filter-grid">
          ...
          <div id="bcAudience" class="muted text-sm mt-md bg-panel border radius-md p-12" aria-live="polite">This will send to <b>–</b> subscribers.</div>
        </div>

        <div class="bc-step" data-step="3">
          <div class="mb-sm"><h3>3. Send time</h3></div>
          <div class="d-flex gap-16 mb-md">
            ...
            <label class="d-flex items-center gap-4 cursor-pointer"><input type="radio" name="bcWhen" value="schedule" /> Schedule</label>
          </div>
        ...
        <div class="bc-step" data-step="4">
          <div class="mb-sm"><h3>4. Preview &amp; test</h3></div>
          <div class="d-flex flex-wrap items-center gap-12">
            <button class="btn btn--outline" data-action="openBroadcastPreview" type="button">Preview broadcast</button>
            ...
            <div class="d-flex gap-8">
              ...
              <button class="btn btn--outline bc-test-action" data-action="testBroadcast" type="button">Send rehearsal to my chat</button>
            </div>
          </div>
          <span id="bcTestChat-error" class="field-err mt-sm" role="alert"></span>
        </div>

        <div id="bcSummary" class="bg-panel border radius-md p-16" hidden>
          <div class="mb-sm"><h3>5. Summary</h3></div>
      ...
      <p class="muted hint text-xs mt-lg">Use <code>{name}</code> to include the subscriber's first name. Get your chat ID by sending <code>/start</code> to <a href="https://t.me/userinfobot" target="_blank" rel="noopener">@userinfobot</a>. Scheduled broadcasts can be cancelled until they start sending.</p>
    </div>

    <div class="lb-widget lb-widget--full">
      <div id="bcDetail" class="bc-detail" role="dialog" aria-modal="true" aria-labelledby="bcDetailTitle" hidden>
        <div class="bc-detail-card">
          <div class="bc-detail-head"><h3 id="bcDetailTitle">Broadcast record</h3><button class="btn btn--ghost" data-action="closeBroadcastDetail" type="button">Close</button></div>
          <div id="bcDetailBody"></div>
        </div>
      </div>
      <div class="v3-table-scroll">
        <table class="v3-table">
          <thead><tr><th>Status</th><th>Subscribers</th><th>Message</th><th>Bot</th><th>Scheduled</th><th>Sent</th><th>Failed</th><th><span class="sr-only">Actions</span></th></tr></thead>

...
const gradientFromHandle = (handle) => { const h = hash(handle); return `linear-gradient(135deg, hsl(${h % 360}, 80%, 60%), hsl(${(h * 13) % 360}, 80%, 40%))`; };
  ...
  function rowWestern(pl, rank, delay, gap) {
    ...
    return `<div class="w-full rounded-lg p-4 flex items-center border-l-8 border-[#F5A623] shadow-[0_4px_10px_rgba(0,0,0,0.5)] transform hover:scale-[1.01] transition-all cursor-pointer hover:bg-[#381E0C] group bg-[#2C1000]"><div class="w-10 h-10 md:w-12 md:h-12 bg-[#F5A623] rounded-full flex items-center justify-center text-[#2C1000] shadow-[inset_0_2px_4px_rgba(255,255,255,0.4)] shrink-0 border-2 border-[#C98415] relative"><div class="absolute inset-0 border-2 border-[#C98415] rounded-full rotate-45 scale-[1.05] opacity-30"></div><span class="font-black text-lg md:text-xl relative z-10 font-['Inter']">${rankStr}</span></div><div class="ml-4 flex-grow flex flex-col md:flex-row md:items-center justify-between"><div class="flex items-center gap-2"><span class="text-[#FFF8E7] text-lg md:text-xl tracking-wide group-hover:text-[#F5A623] transition-colors [font-family:'Rye',_serif]">${name}</span>${icon}</div><div class="text-[#F5A623] text-lg md:text-2xl tabular-nums mt-1 md:mt-0 flex items-center gap-2 [font-family:'Rye',_serif]">${score}<!-- --> <span class="text-base opacity-80 filter grayscale-[50%] brightness-[1.5]">${rankStr}</span></div></div></div>`;
  }

  window.CASINO_BUILDERS = window.CASINO_BUILDERS || { top3: {}, rows: {} };
  window.CASINO_BUILDERS.top3["western"] = top3Western;
  window.CASINO_BUILDERS.rows["western"] = rowWestern;
})();

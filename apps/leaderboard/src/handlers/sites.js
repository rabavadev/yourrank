...
export async function handleGetSite(request, env) {
  ...
  return json({ ok: true, slug: s.slug, published: s.published, plan: plan, data: s.data, socials: s.socials, notify: s.notify || {}, archives: s.archives, boards, siteId: s.id, customDomain: s.customDomain || "", domainStatus: s.domainStatus || "pending", onboarding, templates: templateCatalog(), updatedAt: s.updatedAt }, 200, { "cache-control": "no-store, no-cache, must-revalidate" });
}

...
//   export const handleFoo = withHandler(async (request, env, ctx) => {
//     ...
//   });
//
...
/**
 * @template {(request: Request, env: object, ctx?: object) => Promise<Response>} T
 * @param {T} fn  The actual handler function
 * @returns {T}
 */
export function withHandler(fn) {
  return async function handlerWrapper(request, env, ctx) {
    try {
      ...
      console.error(`[handler:${label}] unhandled error:`, String(err?.message || err));

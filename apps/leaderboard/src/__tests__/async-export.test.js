...
describe("async account export", () => {
  ...
  it("only reports a completed artifact through an owner-scoped job", async () => {
    ...
    const res = await handleExportJobDownload(new Request("https://yourrank.site/api/account/export/job-1/download", { method: "GET" }), env, { slug: "job-1" }, d);
    expect(res.status).toBe(200);
    ...
    expect((await handleExportJobDownload(new Request("https://yourrank.site/api/account/export/job-1/download", { method: "GET" }), env, { slug: "job-1" }, otherOwner)).status).toBe(404);
  });
  ...
  it("refuses expired jobs and surfaces failed status", async () => {
    ...
    expect((await handleExportJobDownload(request("GET"), env, { slug: "job-1" }, expired)).status).toBe(404);

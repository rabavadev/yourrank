...
import type { TelegramBot, TelegramSubscriberStatsResponse } from "@/lib/types";
...
export async function TelegramBotsPage() {
  const [botsResult, statsResult] = await Promise.all([
    ...
    botApiGet<TelegramSubscriberStatsResponse>("/stats/subscribers"),
  ]);

  if (!botsResult.ok) return <ErrorState message={botsResult.error} />;

  const bots = botsResult.data;
  const stats = statsResult.ok ? statsResult.data.totals : { total: 0, active: 0, new_7d: 0, new_30d: 0 };
